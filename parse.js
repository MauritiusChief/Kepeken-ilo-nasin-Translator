...
规则：
1. 拆分依据是句子成分和语义独立性，而非标点符号。
2. 并列的主谓结构或动作要拆成多个句子。
3. 主句+从句保持整体；修饰性从句也保持在主句中。
4. 并列成分（如并列宾语）不拆。
5. 即使省略主语、宾语或谓语，只要能独立表达，也算一个句子。
6. 若输入确实只有一个句子，输出数组可只包含这个句子。
7. 输出严格为：{ "results": ["句子1", "句子2", ...] }；不得输出 JSON 以外内容。
`
...
4) 谓语列
- 将动词及其助动词放入 "谓语"；若有仅修饰动词的修饰词（常为副词）也视为动词的一部分。
- 若有并列动词，分别建立多条谓语项。
- 若存在宾语，从谓语中剥离出来，单独放入 "宾语列"；若存在仅修饰宾语的修饰词（常为副词——比如“只有”——或者形容词），也应视为宾语的一部分。
...
6. "复合修饰词" 数组中的 {头词, 修饰词[]} 结构应当有至少一个修饰词；如果一个 {头词, 修饰词[]} 结构中只有头词，那么将该结构移出 "复合修饰词"，然后将其头词放入上一级的 "修饰词" 数组。
7. 若修饰词用到了 ala, 其应当放置在 "修饰词" 数组的末尾，无论是根目录 "修饰词" 数组还是 "复合修饰词" 内部的 "修饰词" 数组。
8. 不用拘泥于整体翻译时的结果，重点是体现中心词、修饰词与次级修饰词之间的修饰关系。
...
5. 若修饰词用到了 ala, 应当放置在 "修饰词" 数组的末尾。
6. 不用拘泥于整体翻译时的结果，重点是体现前动词、中心动词与修饰词之间的修饰关系。
